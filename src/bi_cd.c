...
/*   Updated: 2022/12/19 00:01:56 by jdias-mo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	cd_dash(t_sh *sh, t_cmd *cmd)
{
	...
	if (cmd->full_cmd[1] && !ft_strncmp(cmd->full_cmd[1], "-", 1) && ft_strlen(cmd->full_cmd[1]) == 1)
	{
		temp = cmd->full_cmd[1];
		...
		ft_putendl_fd(cmd->full_cmd[1], STDOUT_FILENO);
	}
	return (0);
}
